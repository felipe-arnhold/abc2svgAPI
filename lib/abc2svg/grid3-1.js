...
wc=w/nc}else{w=wc*nc}
x0=s.lm<0?(img.width/cfmt.scale-w)/2:s.lm+1
if((s.fmt||cfmt).trimsvg)
...
wc+=abc.strwh('  ')[0]
build_grid()
abc.blk_flush()},do_begin_end:function(of,type,opt,txt){var i,s,v,lm=-1,width
...
if(!i)
break
switch(i[0]){case'c':this.cfmt().csdef=txt
break
case'n':type=""
break
case'l':case'w':v=i.match(/-?[\d.]+.?.?/)
if(v){v=this.get_unit(v[0])
if(!isNaN(v)){if(i[0]=='l')
lm=v
else
width=v}}
...
s.text=txt
s.lm=lm
s.width=width}else{abc2svg.grid3.block_gen.call(this,null,{subtype:type,text:txt,lm:lm,width:width})}}},output_music:function(of){var ln,i,dt,ss,ntim,p_vc,s3,cl,C=abc2svg.C,abc=this,s=abc.get_tsfirst(),vt=abc.get_voice_tb(),t=abc.cfmt().csdef,cs=[]
