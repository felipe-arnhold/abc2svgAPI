...
function ToAudio(){return{add:function(first,voice_tb,cfmt){var toaud=this,C=abc2svg.C,p_time=0,abc_time=0,play_fac=C.BLEN/4*120/60,i,n,dt,d,v,s_m,s=first,rst=s,rst_fac,rsk=[],b_tim,b_typ
...
first.p_s[i]=s}}}}
function relink(s,dt){var tim=s.time+dt,s2=s.ts_next
s.ts_prev.ts_next=s2
s2.ts_prev=s.ts_prev
while(!s2.seqst&&s2.ts_next)
s2=s2.ts_next
if(s2.time<s.time+dt)
dt=s2.time-s.time
s.time+=dt
s.dur-=dt
...
g.pdur=d
t+=d}}
function set_tempo(s){if(!s.tempo){var p=(s.tempo_str1||s.tempo_str2)
if(!p)
return play_fac
...
abc2svg.chord=function(first,voice_tb,cfmt){var chnm,i,k,vch,s,gchon,s_ch,rhy,ti,dt,md=first.p_v.meter.wmeasure,nextim=0,C=abc2svg.C,trans=48+(cfmt.chord.trans?cfmt.chord.trans*12:0)
function chcr(b,ch){var t,r=ch.slice(),i=r.length
...
r[0]-=12
return r}
function meterhy(s){var r,t=s.a_meter[0].top,b=s.a_meter[0].bot
switch(b){case'2':b=4
t*=2
...
function insch(tim){if(s_ch.nhd==undefined)
return
...
s.time=tim
switch(i){case'c':s.notes.shift()
s.nhd--
break
default:i="GHIJKghijk".indexOf(rhy[ti])
if(i<0||i>=s.nhd)
return
...
if(k==9)
k++
vch={v:voice_tb.length,id:"_chord",time:0,sym:{type:C.BLOCK,subtype:"midiprog",chn:k+1,instr:cfmt.chord.prog||0,time:0,dur:0},vol:cfmt.chord.vol||.6}
vch.sym.p_v=vch
vch.sym.v=vch.v
...
ti=0
s=first
while(1){if(gchon&&rhy!='+'){while(s.time>nextim){insch(nextim)
nextim+=dt}}
if(s.a_gch){for(i=0;i<s.a_gch.length;i++){if(s.a_gch[i].type!='g')
continue
gench(s,i)
if(rhy=='+')
insch(s.time)
break}}
if(!s.dur){if(s.bar_num){ti=0}else if(s.wmeasure){md=s.wmeasure
if(rhy!='+')
bld_rhy(meterhy(s))}else if(s.subtype=="midigch"){if(s.on!=undefined)
gchon=s.on
else
bld_rhy(s.rhy)}}
if(!s.ts_next)
