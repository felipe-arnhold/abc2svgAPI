...
if(p_v.second)
ov_def(v)}}
of()},draw_symbols:function(of,p_voice){var s,s2,g,nl,y,C=abc2svg.C,abc=this,dot="\ue1e7",anno_a=abc.anno_a,staff_tb=abc.get_staff_tb(),out_svg=abc.out_svg,out_sxsy=abc.out_sxsy,xypath=abc.xypath
if(!p_voice.jianpu){of(p_voice)
return}
function draw_dur(s1,x,y,s2,n,nl){var s,s3
xypath(x-3,y+5)
...
break
s=s.next}}}
function draw_dgr(s){var g,x,l,y=staff_tb[s.st].y,s2=s.extra,nl=s2.nflags
for(g=s2;g.next;g=g.next);out_svg('<g transform="translate(')
out_sxsy(s2.x,',',y+19)
...
y=(nl>0?5*nl:0)+2
if(nl>0){xypath(-3,0)
while(--nl>=0){out_svg('m0 4h'+l)
l=-l}
out_svg('"/>\n')}
if(s.fmt.graceslurs&&!s.sl1){l=Math.abs(l)
out_svg('<path class="stroke" \
d="m'+(l/2-3).toFixed(1)+' '+y.toFixed(1))
if(s.gr_shift){out_svg('m0 0c0 10 0 10 -10 10')}else{out_svg('m0 0c0 10 0 10 10 10')}
out_svg('"/>\n')}
out_svg('</g>\n')
abc.stv_g().g--}
